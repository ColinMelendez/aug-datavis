/**
 * Generate mesh data equivalent to the Python NumPy code.
...
// Example usage:
// const { X, Y, Z } = generateMeshData();
// console.log(`X shape: ${X.length}x${X[0].length}`);
// console.log(`Y shape: ${Y.length}x${Y[0].length}`);
// console.log(`Z shape: ${Z.length}x${Z[0].length}`);
