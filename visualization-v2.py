...
def plot_data(
    ...
    def process_direction_segments(X_arr, Y_arr, Z_arr, axis, cmap_below, cmap_above):
        """
        Process all segments for one direction (X or Y).
        """
        ...
        if n_same > 0:
            ...
            t_color = np.where(
                is_below,
                bottom_hue_range[0] + bottom_hue_range[1] * t_below,
                top_hue_range[0] + top_hue_range[1] * t_above,
            )
        ...
        if n_cross > 0:
            ...
            # First sub-segment colors (uses below_0_c to determine which cmap)
            t_color_1 = np.where(
                below_0_c,
                bottom_hue_range[0] + bottom_hue_range[1] * t_below_1,
                top_hue_range[0] + top_hue_range[1] * t_above_1,
            )
            ...
            # Second sub-segment colors (opposite of below_0_c)
            t_color_2 = np.where(
                ~below_0_c,
                bottom_hue_range[0] + bottom_hue_range[1] * t_below_2,
                top_hue_range[0] + top_hue_range[1] * t_above_2,
            )
            ...
            # Build crossing segments (interleaved: seg1, seg2, seg1, seg2, ...)
            base_idx = n_same
